                ...
                @foreach ($connections as $app => $readings)
                    <div wire:key="connections:{{ $app }}">
                        ...
                        @php
                            ...
                            $maxSeries = collect($readings['reverb_connections:max'] ?? [])->filter(fn ($v) => $v !== null);
                            $avgSeries = collect($readings['reverb_connections:avg'] ?? [])->filter(fn ($v) => $v !== null);
                            $scale = 1 / $config['sample_rate'];
                            $peak = $maxSeries->max() * $scale;
                            $average = $avgSeries->count() ? ($avgSeries->avg() * $scale) : 0;
                            $current = $maxSeries->last() !== null ? ($maxSeries->last() * $scale) : 0;
                            $approx = $config['sample_rate'] < 1 ? '~' : '';
                        @endphp

                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Past {{ $this->periodForHumans() }} —
                            peak: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $approx }}{{ number_format($peak) }}</span>,
                            avg: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $approx }}{{ number_format($average, 1) }}</span>,
                            latest: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $approx }}{{ number_format($current) }}</span>
                        </div>
