                ...
                @foreach ($messages as $app => $readings)
                    <div wire:key="messages:{{ $app }}">
                        ...
                        @php
                            ...
                            $sentTotal = collect($readings['reverb_message:sent'] ?? [])->sum() * (1 / $config['sample_rate']);
                            $receivedTotal = collect($readings['reverb_message:received'] ?? [])->sum() * (1 / $config['sample_rate']);
                        @endphp

                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Total past {{ $this->periodForHumans() }} —
                            sent: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $config['sample_rate'] < 1 ? '~' : '' }}{{ number_format($sentTotal) }}</span>,
                            received: <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $config['sample_rate'] < 1 ? '~' : '' }}{{ number_format($receivedTotal) }}</span>
                        </div>
